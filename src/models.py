...
class Encoder(nn.Module):
    ...
    def __init__(self, img_channels=1, num_classes=4, latent_dim=32, base_ch=64, img_size=224):
        ...
        self.net = nn.Sequential(
            ...
            nn.Conv2d(c * 8, c * 2, 1), nn.BatchNorm2d(c * 2), nn.LeakyReLU(0.2, inplace=True),
...
class Decoder(nn.Module):
    ...
    def __init__(self, img_channels=1, num_classes=4, latent_dim=32, base_ch=64, img_size=224):
        ...
        for w_in, w_out in zip(widths, widths[1:]):
            stages += [
                nn.Conv2d(w_in, w_out * 4, 3, 1, 1), nn.BatchNorm2d(w_out * 4),
                nn.ReLU(inplace=True), nn.PixelShuffle(2),
                nn.Conv2d(w_out, w_out, 3, 1, 1), nn.BatchNorm2d(w_out),
...
class Discriminator(nn.Module):
    ...
    def __init__(self, img_channels=1, num_classes=4, base_ch=64, img_size=224, norm="batch"):
        ...
        self.net = nn.Sequential(
            *[spectral_norm(m) if isinstance(m, nn.Conv2d) else m for m in layers])
        self.fc = spectral_norm(nn.Linear(c * 8, 1))
